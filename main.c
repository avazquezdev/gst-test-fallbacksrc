...
main (int argc, char *argv[])
{
  ...
  if (!data.pipeline || !data.source || !data.depay ||
		!data.parse || !data.dec || !data.convert ||
		!data.sink) {
   ...
   * point. We will do it later. */
	...
	gst_bin_add_many (GST_BIN (stream), data.source, data.depay, data.parse, data.dec, NULL);
	gst_element_link_many(data.source, data.depay, NULL);
	if (!gst_element_link_many (data.parse, data.dec, NULL)) {
		g_printerr ("Elements stream could not be linked.\n");
		gst_object_unref (data.pipeline);
		return -1;
	}

  gst_bin_add_many (GST_BIN (data.pipeline), stream, data.convert, data.sink, NULL);
  if (!gst_element_link_many (data.dec, data.convert, data.sink, NULL)) {
